...
static func setVolumeForBus(bus_name:String, volume_db:float) -> void:
...
static func getVolumeForBus(bus_name:String) -> float:
	var id = AudioServer.get_bus_index(bus_name)
	return AudioServer.get_bus_volume_db(id)

static func fadeoutAndFree(target:AudioStreamPlayer) -> void:
